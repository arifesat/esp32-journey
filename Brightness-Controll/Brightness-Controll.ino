...
void loop() {
  ...
  if(req.indexOf("/set?value=") != -1){
    ...
    if(val >= 0 && val <= 255){
      brightness = val;
      ledc_set_duty(LEDC_LOW_SPEED_MODE, LED_CHANNEL_0, brightness);
      ledc_update_duty(LEDC_LOW_SPEED_MODE, LED_CHANNEL_0);
...
<body>
  <h2>ESP32 LED Parlaklık Kontrolü</h2>
  <p>Parlaklık: <span id="val">%BRIGHT%</span></p>
